...
def rebalancing():
    ...
    """
    vA = [(1/10) * x for x in range(23,24)] # percentage of tkrA
    perA = [(1/100) * x for x in range(78,79)] # percentage of tkrA
    thr = [(1/100) * x for x in range(10,11)]  # rebalancing threshold
    ...
    for v in vA:
        for x in perA:
            ...
            for y in thr:
                ...
                balB = balStckB * srB[cls]
                bal = balA + balB + balCsh
                dfO = dfO.append({'vA':"%5.2f"%(v),'tkrA':"%5.2f"%(x),'tkrB':"%5.2f"%(perB),'threshold':"%5.2f"%(y),'balance':"%12d"%(bal)}, ignore_index=True) # output Dataframe
                # if (bal > (inv*150)):
