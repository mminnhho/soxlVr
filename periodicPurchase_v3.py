...
buy EVERY MONTH whick rises more by previous HIGH price
cumulative rate 52x
...
for i in range(0, len(dfA)):
	if i != 0:
		...
		month = pd.Period(srA[0], freq='M')
		if (month != previousMonth) or (i == 1):
			cashBalance += cashFlow

			if (srA[4] / previousHighA) <= (srB[4] / previousHighB):  # maxYield 52x
			# if (srA[4] / initialA) >= (srB[4] / initialB):  # maxYield 36x
				quantityA = int(cashBalance / (srA[4] * (1 + feeRate)))
				quantityB = 0
				sumA += quantityA
				fee = (srA[4] * quantityA) * feeRate
				sumFee += fee

			else:
				quantityA = 0
				quantityB = int(cashBalance / (srB[4] * (1 + feeRate)))
				sumB += quantityB
				fee = (srB[4] * quantityB) * feeRate
				sumFee += fee

			cashBalance -= ((srA[4] * quantityA) + (srB[4] * quantityB) + fee)
		...
		dfO = pd.concat([dfO, dfT], ignore_index=False, axis=0)
		previousMonth = pd.Period(srA[0], freq='M')
